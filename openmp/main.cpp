...
#pragma omp target enter data map(alloc:WH[0:N*M], Waux[0:N*K], Haux[0:M*K], acumm_W[0:K], acumm_H[0:K]) \
	...
	for (int iter = 0; iter < niter; iter++) {
		...
		division_t = gettime();
		#pragma omp target teams distribute parallel for num_teams(EU) thread_limit(thread_limit) device(deviceId)
		for(int i = 0; i < N*M; i++)
			WH[i] = V[i] / WH[i]; /* V./(W*H) */
		...
		#pragma omp target teams distribute parallel for device(deviceId)
		for(int i = 0; i < K; i++) {
			acumm_W[i] = 0;
		}

		for (int j = 0; j < K; j++){
			#pragma omp target teams distribute parallel for reduction(+:acumm_W[j]) map(to:j) device(deviceId)
			for (int i = 0; i < N; i++) {
				acumm_W[j] += W[i*K + j];
			}
		}
		...
		for (int j = 0; j < K; j++){
			#pragma omp target teams distribute parallel for reduction(+:acumm_H[j]) map(to:j) device(deviceId)
			for (int i = 0; i < M; i++) {
