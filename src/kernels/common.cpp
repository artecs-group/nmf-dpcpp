...
void accum(queue q, C_REAL *acc, C_REAL *X, int N, int M) {
    ...
    tree_reduction(q, acc, X, data_size, M, work_group_size);
    q.wait();
...
void tree_reduction(queue q, C_REAL *acc, C_REAL *X, int data_size, int work_groups, int work_group_size) {
    q.submit([&](auto &h) {
        ...
        int work_group_size = work_group_range.get(0) * work_group_range.get(1) * work_group_range.get(2);
        int fixed_data_size = work_groups * work_group_size;
        ...
        // h.parallel_for(sycl::nd_range{range(fixed_data_size, 1, 1), work_group_range}, sumr, [=](sycl::nd_item<3> item, auto &sumr_arg) {
        //     int global_id = item.get_global_id(0);
        //     sumr_arg += X[global_id];
        // });

        h.parallel_for(sycl::nd_range(range(fixed_data_size, 1, 1), work_group_range), [=](sycl::nd_item<3> item) {
            ...
            if (local_id == 0 && group_id < work_groups)
